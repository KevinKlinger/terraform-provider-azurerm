...
This example provisions a basic Windows Virtual Machine Scale Set on an internal network. Additional examples of how to use the `azurerm_windows_virtual_machine_scale_set` resource can be found [in the ./examples/vm-scale-set/windows` directory within the Github Repository](https://github.com/kevinklinger/terraform-provider-azurerm/v2/tree/main/examples/vm-scale-set/windows).
