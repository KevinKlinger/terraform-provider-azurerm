...
var _ resourceid.Formatter = VirtualMachineConfigurationAssignmentId{}

func TestVirtualMachineConfigurationAssignmentIDFormatter(t *testing.T) {
	actual := NewVirtualMachineConfigurationAssignmentID("12345678-1234-9876-4563-123456789012", "resGroup1", "vm1", "assignment1").ID()
...
func TestVirtualMachineConfigurationAssignmentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VirtualMachineConfigurationAssignmentId
	}{
	...
	for _, v := range testData {
		...
		actual, err := VirtualMachineConfigurationAssignmentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestVirtualMachineConfigurationAssignmentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *VirtualMachineConfigurationAssignmentId
	...
	for _, v := range testData {
		...
		actual, err := VirtualMachineConfigurationAssignmentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
