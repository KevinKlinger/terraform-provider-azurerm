...
var _ resourceid.Formatter = StreamingJobId{}

func TestStreamingJobIDFormatter(t *testing.T) {
	actual := NewStreamingJobID("12345678-1234-9876-4563-123456789012", "resGroup1", "streamingJob1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/resGroup1/providers/Microsoft.StreamAnalytics/streamingjobs/streamingJob1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestStreamingJobID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *StreamingJobId
	}{
	...
	for _, v := range testData {
		...
		actual, err := StreamingJobID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
