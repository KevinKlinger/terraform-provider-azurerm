...
var _ resourceid.Formatter = ReplicationFabricId{}

func TestReplicationFabricIDFormatter(t *testing.T) {
	actual := NewReplicationFabricID("12345678-1234-9876-4563-123456789012", "group1", "vault1", "fabric1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/group1/providers/Microsoft.RecoveryServices/vaults/vault1/replicationFabrics/fabric1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestReplicationFabricID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ReplicationFabricID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
