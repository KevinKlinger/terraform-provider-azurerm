...
var _ resourceid.Formatter = ReplicationProtectionContainerId{}

func TestReplicationProtectionContainerIDFormatter(t *testing.T) {
	actual := NewReplicationProtectionContainerID("12345678-1234-9876-4563-123456789012", "group1", "vault1", "fabric1", "container1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/group1/providers/Microsoft.RecoveryServices/vaults/vault1/replicationFabrics/fabric1/replicationProtectionContainers/container1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestReplicationProtectionContainerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *ReplicationProtectionContainerId
	}{
	...
	for _, v := range testData {
		...
		actual, err := ReplicationProtectionContainerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
