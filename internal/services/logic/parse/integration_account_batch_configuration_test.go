...
var _ resourceid.Formatter = IntegrationAccountBatchConfigurationId{}

func TestIntegrationAccountBatchConfigurationIDFormatter(t *testing.T) {
	actual := NewIntegrationAccountBatchConfigurationID("12345678-1234-9876-4563-123456789012", "group1", "account1", "batchConfiguration1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/group1/providers/Microsoft.Logic/integrationAccounts/account1/batchConfigurations/batchConfiguration1"
	if actual != expected {
...
func TestIntegrationAccountBatchConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *IntegrationAccountBatchConfigurationId
	}{
...

		{
			...
			Expected: &IntegrationAccountBatchConfigurationId{
				...
				BatchConfigurationName: "batchConfiguration1",
			},
		},
	...
	for _, v := range testData {
		...
		actual, err := IntegrationAccountBatchConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
		...
		if actual.BatchConfigurationName != v.Expected.BatchConfigurationName {
			t.Fatalf("Expected %q but got %q for BatchConfigurationName", v.Expected.BatchConfigurationName, actual.BatchConfigurationName)
		}
	}
}
