...
build-docker:
	mkdir -p bin
	docker run --rm -v $$(pwd)/bin:/go/bin -v $$(pwd):/go/src/github.com/kevinklinger/terraform-provider-azurerm/v2 -w /go/src/github.com/kevinklinger/terraform-provider-azurerm/v2 -e GOOS golang:1.16 make build
...
test-docker:
	docker run --rm -v $$(pwd):/go/src/github.com/kevinklinger/terraform-provider-azurerm/v2 -w /go/src/github.com/kevinklinger/terraform-provider-azurerm/v2 golang:1.13 make test
...
testacc: fmtcheck
	TF_ACC=1 go test $(TEST) -v $(TESTARGS) -timeout $(TESTTIMEOUT) -ldflags="-X=github.com/kevinklinger/terraform-provider-azurerm/v2/version.ProviderVersion=acc"

acctests: fmtcheck
	TF_ACC=1 go test -v ./internal/services/$(SERVICE) $(TESTARGS) -timeout $(TESTTIMEOUT) -ldflags="-X=github.com/kevinklinger/terraform-provider-azurerm/v2/version.ProviderVersion=acc"
